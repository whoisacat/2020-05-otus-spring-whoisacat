...
import com.whoisacat.edu.book.jpa.catalogue.repository.AuthorRepository;
...
class AuthorServiceSimpleTest{
    ...
    void foundByNameOneAuthor(){
        when(repository.getByName("name")).thenReturn(Lists.newArrayList(AUTHOR_2));
        assertThat(service.findByNameOrCreate("name")).isEqualTo(AUTHOR_2);
    }

    @Test
    void findByNameOrCreate(){
        when(repository.getByName("name")).thenReturn(Lists.newArrayList(AUTHOR_2,AUTHOR_1_WITH_BOOK));
    ...
    void foundNoOneAuthorByName(){
        when(repository.getByName("name")).thenReturn(new ArrayList<>());
        assertThat(service.findByName("name")).isEqualTo(AuthorServiceSimple.NOT_FOUND);
    }

    @Test
    void foundCoupleOfAuthorsByName(){
        when(repository.getByName("name")).thenReturn(Lists.newArrayList(AUTHOR_1_WITH_BOOK,AUTHOR_2));
